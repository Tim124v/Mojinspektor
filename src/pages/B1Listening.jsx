...
import { examStructure, listeningTasks } from '../data/b1ExamData'
import useExamTimer from '../hooks/useExamTimer'
import { useTranslation } from '../hooks/useTranslation'

export default function B1Listening() {
  ...
  const task = listeningTasks[taskIdx]
  const item = task.type === 'context_recognition' ? task.items[itemIdx] : null
  const totalPoints = useMemo(() => listeningTasks.reduce((sum, t) => sum + (t.items?.length || 0), 0), [])

  function nextTask() {
    if (taskIdx >= listeningTasks.length - 1) {
      const prev = b1Progress.listening
      const sectionMax = examStructure.sections.find((s) => s.id === 'listening')?.maxPoints || 30
      const normalizedScore = Math.round((score / totalPoints) * sectionMax)
      updateB1Progress('listening', {
  ...
  return (
    <div className="px-4 py-4">
      ...
      <p className="mb-1 text-xs text-gray-500">
        {t.b1_task} {taskIdx + 1}/{listeningTasks.length}
      </p>
      ...
      {task.type === 'context_recognition' && (
        <>
          <div className="mb-2 text-xs text-gray-500">
            {t.b1_question} {itemIdx + 1}/{task.items.length}
          </div>
          <div className="mb-3 rounded-xl border border-gray-100 bg-white p-4 text-sm">{item.transcript}</div>
          ...
          <button
            onClick={checked ? nextChoice : checkChoice}
            disabled={timer.isExpired && !checked}
      ...
      {task.type === 'tak_nie' && (
        <>
          <div className="mb-3 rounded-xl border border-gray-100 bg-white p-4 text-sm whitespace-pre-wrap">{task.transcript}</div>
          <div className="mb-4 flex flex-col gap-2">
            {task.items.map((row) => {
          ...
          <button
            onClick={takNieChecked ? nextTask : checkTakNie}
            disabled={timer.isExpired && !takNieChecked}
            className="w-full rounded-xl bg-primary py-3 font-semibold text-white disabled:opacity-50"
          >
            {takNieChecked ? t.b1_finish_listening : t.b1_check}
          </button>
        </>
      )}
    </div>
